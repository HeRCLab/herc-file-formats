...
//export MLPXIsomorphicDuplicate
func MLPXIsomorphicDuplicate(sourceHandle C.int, destHandle *C.int, snapid *C.char) C.int {
	mlp := getMLP(sourceHandle)
	if mlp == nil {
		return 1
	}

	dup, err := mlp.IsomorphicDuplicate(C.GoString(snapid))
	if err != nil {
